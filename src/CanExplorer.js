...
export default class CanExplorer extends Component {
	...
	spawnWorker(options) {
		...
		worker.onmessage = (e) => {
			console.log(`[CanExplorer::worker.onmessage] e: `, JSON.stringify(e));
			console.log(`[CanExplorer::worker.onmessage] e.data: `, JSON.stringify(e.data));
			console.log(`[CanExplorer::worker.onmessage] e.data.newMessages: `, JSON.stringify(e.data.newMessages));
	...
	addAndRehydrateMessages(newMessages, options) {
		console.log(`[CanExplorer::addAndRehydrateMessages], newMessages: `, JSON.stringify(newMessages));
		debugger;
	...
	showingModal() {
		const { showOnboarding, showLoadDbc, showSaveDbc, showAddSignal, showEditMessageModal } = this.state;
	...
	render() {
		...
		return (
			<div id="cabana" className={cx({ "is-showing-modal": this.showingModal() })}>
				{this.state.isLoading ? <LoadingBar isLoading={this.state.isLoading} /> : null}
					...
					<div className="cabana-header-account">
						{this.state.isGithubAuthenticated ? (
							<div>
								...
								<p className="cabana-header-account-signout" onClick={this.githubSignOut}>
									Sign out
								</p>
							</div>
						) : (
