...
export function uint64BEToHex(int64: number) {
	return new Uint64BE(int64).toString(16);
}

export function int64BufferToPrettyHexStr(buffer: Buffer) {
	const uint = new Uint64BE(buffer);
...
export function formatMsgHex(msg: unknown) {
	// @ts-ignore
	const uint = new Uint64BE(msg[1]);
	let hex = uint.toString(16);
	if (hex.length === 1) {
		hex = `0${hex}`;
	}
